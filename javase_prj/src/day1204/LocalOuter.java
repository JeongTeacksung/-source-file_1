	...
	public void method(int param_i, final int param_j ) {
		...
		class Local{
			...
			public void locMethod() {
				...
				//final이 붙은 매개변수만 사용할 수 있다 (JDK1.8버전부터 사용가능)
				System.out.println("매개변수(Parameter) param_i="+
						param_i+", param_j="+param_j);			
				//final이 붙은 지역변수만 사용할 수 있다 (JDK1.8버전부터 사용가능)
				System.out.println("지역변수 local_i="+local_i+", local_j="
						+local_j);
//				local_i=2018; //JDK1.8에서부터는 final이 붙어있지않는
				//지역변수의 출력은 가능하지만 값 할당은 되지 않는다.
