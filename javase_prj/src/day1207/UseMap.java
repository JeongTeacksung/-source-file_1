...
public class UseMap {
	public UseMap(){
		...
		//2. 값 할당 - 키는 유일, 값은 중복가능, 입력되는 순서대로 값이 추가되지 않는다
		...
		map.put("HTML", "Markup Languae");// 키가 존재한다면 해당 키에 덮어 쓴다
		...
		while(ita.hasNext()) {//키가 존재한다면
			value=map.get(ita.next());; // 얻너낸 키를 가지고 map의 값을 얻는다
...
//		if(value==null) { //얻어진 값을 가지고 비교
		if(!bloodMap.containsKey(key)) {//키가 존재하는지?
			System.out.println(key+"형은 사람의 것이 아닙니다.");
		}else {
